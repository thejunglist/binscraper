...
(async () => {
  ...
  await page.goto(
    "https://www.west-dunbarton.gov.uk/recycling-and-waste/bin-collection-day/",
  );
  ...
  if (pageContent.roundInfoCount > 0) {
    ...
    const binData = await page.evaluate(() => {
      ...
      rounds.forEach((round) => {
        try {
          ...
          const dayElement = round.querySelector(
            "div:nth-child(3) > span:nth-child(1)",
          );
          const dateElement = round.querySelector(
    ...
    const roundName = await page
      .$eval("div.round-info:nth-child(1) > div:nth-child(2)", (el) =>
        el.textContent.trim(),
      )
    ...
    const day = await page
      .$eval(
        "div.round-info:nth-child(1) > div:nth-child(3) > span:nth-child(1)",
    ...
    const date = await page
      .$eval(
        "div.round-info:nth-child(1) > div:nth-child(3) > span:nth-child(2)",
        (el) => el.textContent.trim(),
      )
      .catch((e) => console.log("Error getting date:", e.message));

    console.log(
      "Directly extracted: bin type:",
      roundName,
      "on",
      day,
      "the",
